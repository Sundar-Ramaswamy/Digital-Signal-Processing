...
if {! [file exists $base_dir/MCC150_TransceiverBPSK_Decimation]} {
    file delete -force $base_dir/MCC150_TransceiverBPSK_Decimation
}

project new $base_dir MCC150_TransceiverBPSK_Decimation
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/MCC150/MCC150_TransceiverBPSK_Decimation_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/MCC150/MCC150_TransceiverBPSK_Decimation_atb.vhd
vcom -quiet $base_dir/MCC150/MCC150_TransceiverBPSK_Decimation_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_I_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_I_out $cyclenum
            }
            if { [catch {exa mismatch_out_2_Q_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_Q_out $cyclenum
            }
            if { [catch {exa mismatch_out_5_I_adj} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_I_adj $cyclenum
            }
            if { [catch {exa mismatch_out_6_Q_adj} mismatch] == 0 && $mismatch } {
                report_mismatch out_6_Q_adj $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473  -voptargs="+acc" -t ps MCC150_TransceiverBPSK_Decimation_atb 
    do $base_dir/MCC150/MCC150_TransceiverBPSK_Decimation_atb.wav.do
